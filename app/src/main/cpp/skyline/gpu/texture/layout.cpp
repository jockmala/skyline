...
namespace skyline::gpu::texture {
                ...
                deswizzleBlock(linearRob, [&](u8 *linearSector, size_t xT) __attribute__((always_inline)) {
                    ...
                    for (size_t pixelOffset{}; pixelOffset < SectorWidth; pixelOffset += formatBpb) {
                        if (xT < blockPaddingOffset)
                            if constexpr (BlockLinearToLinear)
                                std::memcpy(linearSector + pixelOffset, sector, SectorWidth);
                            else
                                std::memcpy(sector, linearSector + pixelOffset, SectorWidth);
    ...
    void CopyBlockLinearToLinear(Dimensions dimensions, size_t formatBlockWidth, size_t formatBlockHeight, size_t formatBpb, size_t gobBlockHeight, size_t gobBlockDepth, u8 *guestInput, u8 *linearOutput) {
        CopyBlockLinearInternal<true>(
            dimensions,
            formatBlockWidth, formatBlockHeight, formatBpb,
            gobBlockHeight, gobBlockDepth,
            guestInput, linearOutput
        );
    }
    ...
    void CopyLinearToBlockLinear(Dimensions dimensions, size_t formatBlockWidth, size_t formatBlockHeight, size_t formatBpb, size_t gobBlockHeight, size_t gobBlockDepth, u8 *linearInput, u8 *guestOutput) {
        CopyBlockLinearInternal<false>(
            dimensions,
            formatBlockWidth, formatBlockHeight, formatBpb,
            gobBlockHeight, gobBlockDepth,
            guestOutput, linearInput
        );
    }
